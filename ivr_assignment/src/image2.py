...
class image_converter:
  ...
  def callback2(self,data):
    ...
    x_e = self.forward_kin(cv_image2)
    x_e_image = self.detect_end_effector(cv_image2)
    self.end_effector=Float64MultiArray()
    self.end_effector.data= x_e_image	

    # send control commands to joints (lab 3)
    q_d = self.control_closed(cv_image2)
    self.joint1=Float64()
    ...
    x_d = self.trajectory(cv_image2)    # getting the desired trajectory
